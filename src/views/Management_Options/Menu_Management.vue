            ...
            <!-- 세부정보 상세 시작 -->
              ...
              <b-row>
                ...
                <div class="col-md-6 col-sm-6 col-xs-6">
                  ...
                  <b-form-invalid-feedback :state="MenuName_EmptyValidation" class="col-lg-11 col-md-11 col-sm-11 col-xs-11 feedback_style">
                    메뉴명은 필수 입력입니다.
              ...
              <b-row>
                <div class="col-md-6 col-sm-6 col-xs-6">
                  ...
                  <b-form-invalid-feedback :state="MenuRoute_EmptyValidation" class="col-lg-11 col-md-11 col-sm-11 col-xs-11 feedback_style">
                    메뉴경로는 필수 입력입니다.
                  </b-form-invalid-feedback>
                </div>
                <div class="col-md-6 col-sm-6 col-xs-6">
    ...
    created() {
      ...
      this.gridProps = {
        ...
        columns: [
          ...
          { header: "메뉴 풀 코드",  name: "MENU_FULL_CD",   hidden: true },
          { header: "정렬순서",      name: "SORT_NO",        hidden: true },
          { header: "비고",          name: "REMARK",         hidden: true },
          { header: "사용여부",      name: "USE_YN",         hidden: true },
          { header: "상위 코드",     name: "PARENT_MENU_CD", hidden: true },
          { header: "상위 풀코드",   name: "PARENT_MENU_FULL_CD", hidden: true },
          { header: "디바이스 타입", name: "DEVICE_TYPE", hidden: true },
        ...
        treeColumnOptions:{
          name:"MENU_CD", // Key가 될 컬럼 Name (반드시 보여야 한다. hidden:false시 트리 구성 안됨)
    ...
    methods: {
      ...
      Tree_DataConvert(Search_Menu) {
        ...
        for (let idx = 0; idx < Search_Menu.length; idx++) {
          ...
          // 최상위 행일경우 바로 추가
          if (Data_Node.PARENT_MENU_CD == 0) { 
            this.Real_Node.push(Data_Node);       // Real_Node 배열의 끝에 Data_Node 추가
          }
          // 조회 버튼 클릭 시
          else {
            // 부모 행 찾기 (i번째 데이터의 부모 코드, 이미 추가된 바인딩 데이터)
            const Parent_Node = this.Find_Parent_Node(Data_Node.PARENT_MENU_CD, this.Real_Node);
      ...
      Find_Parent_Node(Parent_Code, Data_Set) {
        ...
        for (let idx = 0; idx < Data_Set.length; idx++) { 
          ...
          if (Parent_Code == Data_Set[idx].MENU_CD) {
      ...
      btn_Add() {
        ...
        if (this.$refs.tuiGrid.invoke("getRowCount") != 0) {
          ...
          const Default_Data = {                                               // 행 추가 시 기본 값 
            ...
            REMARK: "",                                         // 비고
            USE_YN: "",                                        // 사용 유무
            PARENT_MENU_CD: Focus_Data_info.MENU_CD,            // 부모 코드
      ...
      async btn_Save() {
        ...
        if(msgBox) {
          ...
          //JSON.stringify            
          const Save_Data={data}; // 저장할 데이터를 담는 변수(메뉴코드, 메뉴명, 메뉴경로, 정렬순서, 비고, 사용여부, 등록/수정자, 메뉴풀코드, 부모메뉴풀코드)
          ...
          if(Result[0].query_success == "N") {
            ...
            if(!Utility.fn_IsNull(this.Real_Node)) {
              ...
              this.$refs.tuiGrid.invoke("focus", findednode.rowKey, "MENU_CD");   // 포커스 적용
            }
      ...
      async btn_Delete() {
        ...
        try {
          ...
          if(!Utility.fn_IsNull(Utility.fn_ObjtoStr(Focus_Data_Index))) {
            ...
            if(!Utility.fn_IsNull(Focus_Data_info.MENU_CD)) {
              const msgbox = await this.$bvModal.msgBoxConfirm("선택된 행을 삭제하시겠습니까?", GlobalValue.Info_option);
              
              if(msgbox){

                const data=[];
                data[0]= {data: JSON.stringify({ MENU_CD:Focus_Data_info.MENU_CD})}
      ...
      async grid_focusChange(DataRow) {
        ...
        this.txt_Menu_Code = DataRow_info.MENU_CD;   // 메뉴 코드
        this.txt_Menu_Name = DataRow_info.MENU_NAME; // 메뉴 명
